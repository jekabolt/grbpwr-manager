...
// (L-4), паттерн — плитка ткани, не состояние изделия: обоим колорвей ОТКАЗЫВАЮТ, а не молча
// обнуляют.
func DesignPictureKindTakesColorway(kind string) bool {
	switch DesignKindOrFlat(kind) {
	case DesignPictureKindRender, DesignPictureKindThreed:
		return true
	}
	return false
}
...
// (L-3). Флэт, вектор, паттерн и текст цвета не имеют — названный там колорвей был бы записью,
// которую ни один читатель не смог бы честно истолковать.
func DesignRunKindTakesColorway(kind string) bool {
	switch kind {
	case DesignRunKindRender, DesignRunKindThreed, DesignRunKindRecolor:
		return true
	}
	return false
}
