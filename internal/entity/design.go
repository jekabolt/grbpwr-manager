...
// DesignConstructionMaxTokens — ПОТОЛОК ОТВЕТА структурного черновика (max_tokens на проводе).
//
// ⚠ ЖИВЁТ ЗДЕСЬ, А НЕ У ДВЕРИ, ПОТОМУ ЧТО ИЗ НЕГО СЧИТАЮТСЯ ТРИ РАЗНЫЕ ВЕЛИЧИНЫ В ТРЁХ ПАКЕТАХ,
// И ДВЕ ИЗ НИХ — ДЕНЬГИ И ВРЕМЯ:
//
//  1. ЦЕНА (apisrv/admin: designDraftIdeaConstructionBaseUSD) — поставщик вправе напечатать
//     потолок целиком, значит верхняя граница рода считается по нему;
//  2. СРОК ВЫЗОВА (openrouter.CompletionBudget) — потолок покупает себе время печати;
//  3. ЛИЗА ХЕНДЛЕРА (store/design.HandlerLeaseFor) — она обязана ПЕРЕЖИВАТЬ пункт 2, иначе строку
//     перехватывает второй хендлер того же client_request_id, пока первый ещё внутри платного
//     вызова, и одно нажатие платит дважды. ⚠ ЛИЗА ВЫВЕДЕНА НЕ ИЗ ЭТОГО ЧИСЛА, А ИЗ МАКСИМУМА
//     DesignDraftAnswerCeilings, в котором оно сегодня наибольшее: анкер на одной ветке разошёлся
//     бы с проводом молча в день, когда потолок появится у соседней. И НЕ ИЗ КОДОВОЙ БАЗЫ СРОКА:
//     базу пункта 2 задаёт OPENROUTER_HTTP_TIMEOUT, поэтому лизу считает тот, кто держит клиента
//     (apisrv/admin), и приносит её в entity.DesignRunStart.HandlerLease.
//
// ПОЧЕМУ НЕ ПРОСЬБА В КОММЕНТАРИИ. Она уже не удержала здесь дважды: потолок подняли 3000 → 8000
// в одиночку, и цена не двинулась ни на цент (круг 21), а срок вызова оборвал ответ на 60-й
// секунде (круг 22). Третьим не удержала и лиза: 5 минут были верны при 60 s на вызов и стали
// ложью на 26.7 s, когда бюджет вызова вырос до 5m26.667s. Число, из которого выводят деньги и
// время В ТРЁХ ПАКЕТАХ, обязано быть ОДНИМ, а не тремя копиями с просьбой ходить строем.
//
// ЗАМЕР, ИЗ КОТОРОГО ВЗЯТО 8000, ЖИВЁТ У ЕГО ПОТРЕБИТЕЛЯ — apisrv/admin:designConstructionMaxTokens
// (таблица четырёх заполненных ответов): он про ФОРМУ ответа, которую entity не знает и знать не
// должен.
const DesignConstructionMaxTokens = 8000

// DesignDraftAnswerCeiling — ПОТОЛОК ОТВЕТА ТОЙ ВЕТКИ нажатия «черновик», которую выбрали; ноль
// значит «потолка нет».
//
// ⚠ ЖИВЁТ ЗДЕСЬ, А НЕ У ДВЕРИ, И ПЕРЕЕХАЛА СЮДА РАДИ ЛИЗЫ. Пока она стояла в apisrv/admin, лиза
// хендлера (store/design.HandlerLeaseFor) не могла её спросить через границу пакетов и была выведена
// из ОДНОЙ ветки — из DesignConstructionMaxTokens. Сегодня это то же число (у прозы потолка нет),
// а в день, когда у прозы появится СВОЙ потолок выше 8000, бюджет вызова обогнал бы лизу и один
// client_request_id снова заплатил бы дважды — МОЛЧА, потому что проба спрашивала бы константу
// одной ветки. Теперь лиза выводится из МАКСИМУМА по DesignDraftAnswerCeilings, и новая ветка
// попадает в неё тем же коммитом, которым заводится.
//
// ⚠ ПОТОЛОК У ПРОЗЫ — ЭТО ТРИ РЕШЕНИЯ ВЛАДЕЛЬЦА, А НЕ ОДНА КОНСТАНТА, и ноль здесь не заглушка.
// Первое: потолок меняет БАЙТЫ ЗАПРОСА, а байты прозаической ветки — контракт со старым клиентом
// (V-19: «отсутствующий флаг обязан давать прежние байты»). Второе: кто ставит потолок, тот
// выключает мышление (openrouter). Третье: с потолком приходит `finish_reason=length`, на который
// прозаическая ветка сегодня не умеет отвечать. Цена прозаической ветки выведена из этого нуля
// (apisrv/admin: designDraftIdeaProseBaseUSD), и TestProseBaseStillCostsWhatTheLiteralDid краснеет
// в тот день, когда он перестанет быть нулём.
...
// DesignDraftAnswerBranches — ВСЕ ФОРМЫ ОДНОГО НАЖАТИЯ, каждая из которых кладёт на провод свой
// потолок. Функция, а не срез-переменная: экспортированный срез правится из любого пакета, а
// таблица, из которой выводится ДЕНЕЖНАЯ величина, обязана быть только читаемой.
func DesignDraftAnswerBranches() []bool { return []bool{false, true} }

// DesignDraftAnswerCeilings — потолок КАЖДОЙ ветки, по одному на ветку и в её порядке.
//
// ⚠ ЭТО И ЕСТЬ «ВСЁ, ЧТО МОЖЕТ ПОПАСТЬ НА ПРОВОД», и читатели обязаны спрашивать ЕЁ, а не
// отдельную константу: всякий, кто выводит из потолка время или деньги, выводит их из МАКСИМУМА
// этого списка. Ветка, заведённая без строки здесь, — это ветка, чью лизу никто не посчитал.
func DesignDraftAnswerCeilings() []int {
	branches := DesignDraftAnswerBranches()
	out := make([]int, 0, len(branches))
	for _, construction := range branches {
		out = append(out, DesignDraftAnswerCeiling(construction))
	}
	return out
}
...
// DesignErrorCodeNoFrontRender — «рендер-верстак этого колорвея занят, но не спереди» (J-26).
//
// ⚠ ЭТО НЕ ВТОРОЕ ИМЯ ДЛЯ `no_fabric_render`, И РАЗЛИЧАТЬ ИХ ОБЯЗАН ЭКРАН. Первый значит «на этом
// верстаке нет НИЧЕГО» и отправляет человека рендерить; этот значит «есть, но не та сторона» и
// отправляет его положить кадр во ФРОНТ — две разные двери и два разных следующих жеста. Одно имя
...
type DesignBenchSlot struct {
	...
	// RunKind / RunRrev — ШТАМП ПРОГОНА, ИЗ КОТОРОГО ВЫШЛА ПЛИТА, разрешаемый тем же чтением,
	// что и сама плита (attachSlotPictures). `db:"-"` — их нет в design_bench_slot, они приходят
	// из design_run по picture.run_id.
	//
	// ЗАЧЕМ, ЕСЛИ У ПЛИТЫ ЕСТЬ RunId. Потому что строка прогона клиенту НЕ ВЫДАЁТСЯ: полоса везёт
	// ПЕРВУЮ СТРАНИЦУ ленты (DefaultRunPageLimit = 12), а плита в слоте, по собственному слову
	// контракта, «routinely older than the first page of runs». То есть id есть, а строки за ним
	// нет, и клиент, ищущий rrev своей стороны, находит НОЛЬ на любой карточке с историей.
	//
	// ЧЕМ ЭТО СТОИЛО. Отказ 3D «four sides of ONE revision» считается по этим rrev. Со всеми
	// нулями множество ревизий ПУСТО, отказ не срабатывает НИКОГДА, и стол собирается из переда
	// одной ревизии и спины другой — оплаченная модель из двух разных окрасок вещи, `done` на
	// строке и ничего в истории, чтобы это различить. Сторож, которому подают нули, хуже
...
type DesignBand struct {
	...
	// ColourPlan — ЦВЕТОВОЙ ПЛАН КАРТОЧКИ (0364) либо nil, если его нет.
	//
	// ⚠ nil И ПУСТОЙ ПЛАН — РАЗНЫЕ ОТВЕТЫ, И РАЗНИЦА НЕСУЩАЯ. Пустой план это «покрасили и стёрли»,
	// состояние, сделанное человеком руками и защищаемое тем же CAS; nil это «здесь никто не
	// рисовал». Подмена одного другим сообщила бы клиенту rev 0 у несуществующей строки, и первое
...
type DesignRunStart struct {
	...
	ColorwayStated bool
	// HandlerLease — СКОЛЬКО ВРЕМЕНИ СТРОКА ПРИНАДЛЕЖИТ ТОМУ, КТО ЕЁ ОТКРЫЛ, у рода draft_idea.
	// Обязательна для него и игнорируется всеми остальными: их строки исполняет воркер, и захват
	// им выдаёт ClaimRuns своей лизой.
	//
	// ⚠ ЕЁ СЧИТАЕТ ВЫЗЫВАЮЩИЙ, И ЭТО НЕ ПЕРЕКЛАДЫВАНИЕ ОТВЕТСТВЕННОСТИ, А ЕДИНСТВЕННОЕ МЕСТО, ГДЕ
	// ОБА ЧИСЛА ВИДНЫ СРАЗУ. Лиза обязана переживать платный вызов; длину вызова знает только тот,
	// кто держит клиента поставщика (его база — openrouter.Client.CompletionBase, его потолок —
	// DesignDraftAnswerCeilings). Стор не видит конфигурацию процесса и видеть её не должен —
	// поэтому, пока он считал лизу САМ, он считал её от КОДОВОЙ базы в 60 s, тогда как на проводе
	// стояла настроенная: при OPENROUTER_HTTP_TIMEOUT = 240 s вызов занимал 506.67 s против лизы в
	// 416.67 s, и повтор того же client_request_id платил модели второй раз.
	//
	// ⚠ НОЛЬ У draft_idea — ОТКАЗ (ErrDesignInvalidArgument), А НЕ ТИХОЕ УМОЛЧАНИЕ. Умолчание
	// вернуло бы ровно тот дефект, который поле чинит: незаполненное поле стало бы кодовой базой,
	// и разошлись бы они снова молча. Считается это поле через store/design.HandlerLeaseFor.
