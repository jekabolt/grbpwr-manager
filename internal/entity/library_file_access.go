...
//
// СТРОКА ЖИВЁТ ДОЛЬШЕ УРОВНЯ. Переключили файл обратно в `team` — строка остаётся с прежним epoch.
// Старая ссылка НЕ оживает при возврате в `link` только потому, что маршрут сверяет
// `access_level = 'link'` на самой строке файла, а не наличие этой строки. Возврат без ротации —
// сознательное «включить ту же ссылку снова».
