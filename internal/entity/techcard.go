...
//
// ЖИВЁТ В entity, А НЕ В dto, потому что это факт о СХЕМЕ (применена ли 0346), а не о форме
// провода: dto знает, как выглядит запрос, а не что умеет chk_tech_card_media_kind. Любой
// будущий серверный писатель `kind` обязан читать ЭТОТ флаг, а не заводить свой, — два флага
// разойдутся ровно в тот день, когда один флипнут, а другой забыли.
//
