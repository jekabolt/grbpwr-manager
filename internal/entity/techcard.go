...
const (
	...
	// TechCardMediaKindDictExtended ниже. Здесь они перечислены как значения ЭНУМА (зеркало proto),
...
//
// ОДНА ПЕРЕМЕННАЯ — ВЕСЬ ГЕЙТ. Поставь true, когда 0346 уехала на ПРОД (бинарь один на оба
// контура), и три новых вида начинают приниматься с провода и писаться сервером; до тех пор они
...
// картинку.
//
// ФЛАГ СУЩЕСТВУЕТ, ПОТОМУ ЧТО 0346 — ОТДЕЛЯЕМЫЙ ФАЙЛ: единственный ADD CHECK волны, то есть
// единственная КОПИЯ таблицы под пятиминутным потолком всего прогона миграций. Файл заведён
// отдельно ровно затем, чтобы его можно было вынуть из волны одним движением и увезти в тихое
// окно, — а тогда бинарь уезжает на прод раньше словаря, и принимать side_l ему нельзя.
//
// ЖИВЁТ В entity, А НЕ В dto, ПОТОМУ ЧТО ЧИТАТЕЛЕЙ ДВА. Кроме входного гейта провода его читает
// DesignPlateMediaKind: атомарный минт вкладывает плиты верстака в technical-медиа карточки
// СЕРВЕРОМ (П-А), то есть выбирает kind без участия провода. Второй флаг разошёлся бы с первым
// ровно в тот день, когда один флипнут, а другой забыли.
//
...
// OperationBomQty — сколько единиц ОДНОГО артикула тратит ОДИН шаг на изделие (0334).
//
// QtyPerGarment — обычный decimal, а не NullDecimal, и это несущее: связь БЕЗ числа в список
// просто не попадает, поэтому «валидное отсутствие» здесь невыразимо по построению. NullDecimal
// завёл бы второе написание одного и того же состояния — запись с Valid=false рядом с отсутствием
// записи, — и первый же читатель, забывший проверить .Valid, прочитал бы «не сказано» как ноль.
//
// ЧИСЛО НЕ ИМЕЕТ НИЧЕГО ОБЩЕГО С PlacementCount ШАГА, и выводить одно из другого запрещено в обе
// стороны: «обметать 6 петель» повторяется шесть раз и не тратит НИ ОДНОЙ пуговицы (петля тратит
// нитку), «поставить 4 хольнитена» тратит два разных артикула по своему числу каждый, «втачать
// молнию» не повторяется вовсе и тратит одну молнию.
type OperationBomQty struct {
	// LineKey — ключ строки BOM, ОБЯЗАННЫЙ присутствовать в BomLineKeys того же шага.
	LineKey string
	// QtyPerGarment — всегда задано (см. шапку типа).
	QtyPerGarment decimal.Decimal
}
...
type TechCardOperation struct {
	...
	BomIds      []int    `db:"-"`
	// BomQuantities — СКОЛЬКО ЕДИНИЦ каждого из этих артикулов шаг тратит на изделие (0334).
	// Разрежённый список, а не третий параллельный срез рядом с BomLineKeys/BomIds: связь без
	// числа сюда не попадает вовсе, поэтому позиции с двумя списками выше НЕ СОВПАДАЮТ и
	// связываться с ними обязаны по ключу. Позиционная связь трёх срезов расходилась бы молча —
	// ровно тот класс, ради которого на проводе заведён ключевой список, а не repeated Decimal.
	//
	// Членство в связи определяет ТОЛЬКО BomLineKeys, единственный владелец; здесь навешиваются
	// числа на уже существующие связи. Ключ, которого нет в BomLineKeys, конвертер отвергает.
	BomQuantities []OperationBomQty `db:"-"`
...
type TechCardInsert struct {
	...
	// GarmentDescription — ЧТО ЭТО ЗА ИЗДЕЛИЕ, СЛОВАМИ ДИЗАЙНЕРА (0348, требование владельца W-3):
	// «oversized boxy shirt, dropped shoulder, no placket». Уезжает в КАЖДЫЙ прогон полосы DESIGN и
	// замерзает там как DesignInputSnapshot.garment_note.
	//
	// ЭТО НИ Concept, НИ MoodNote, и сливать их нельзя. Concept — замысел стиля, свойство ДОКУМЕНТА;
	// MoodNote — про что ДОСКА, её читает ровно один текстовый прогон; это — что такое ИЗДЕЛИЕ, и
	// его читает каждая генерация. Одно поле на все три означало бы, что правка промпта правит
	// заявленный замысел документа.
	//
	// GarmentDescriptionOmitted — поле ОТСУТСТВОВАЛО на проводе, а не «пришло пустым»: тот же
	// verbatim-протокол и та же причина, что у MoodNote выше (админка — SPA, карточка сохраняется
	// ЦЕЛИКОМ, вкладка со старым бандлом про поле не знает и прислала бы ""). Три состояния:
	//   * поле ОТСУТСТВУЕТ (null) → НЕ ТРОГАТЬ (стор: IF(:garment_description_omitted, …));
	//   * поле ЕСТЬ, значение ""  → ОЧИСТИТЬ (в колонку уезжает NULL);
	//   * поле ЕСТЬ со значением  → записать.
	//
	// В проекцию дайджеста НЕ ВХОДИТ, по тому же доводу, что и MoodNote: это подсказка модели, а не
	// факт об изделии, и добавление её в проекцию объявило бы каждую подписанную секцию
	...
	// BomQtyAware — то же самое про количества на связях шага с артикулом (0334,
	// TechCardOperation.BomQuantities). Транспорт, не содержание; НИ В ОДИН ДАЙДЖЕСТ НЕ ВХОДИТ:
	// которым бандлом сохранили карточку — не то, от чего может зависеть подпись. Захешируй его —
	// и подпись CONSTRUCTION протухала бы у КАЖДОЙ карточки в день выкатки клиента, с
	// формулировкой «секция отредактирована после подписания», которую никто не связал бы с
	// причиной.
	//
	// Флаг НЕ ФИЛЬТРУЕТ ПОЛЯ: разбор bom_quantities идёт всегда, независимо от него — по тому же
	// доводу, что у AssemblyAware выше (клон сезона строит payload сам и флагов не эмитит).
	// Парного `*_cleared` у него нет: «количество стёрли» — рядовая правка одной связи, и бекстоп
	// сделал бы её нестираемой.
	BomQtyAware bool `db:"-"`
