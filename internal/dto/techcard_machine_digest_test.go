...
// --- заморозка снятых полей конструкции ----------------------------------------------------------

// ПОЗИЦИИ 3 И 5 КОРТЕЖА КОНСТРУКЦИИ — КОНСТАНТЫ, А НЕ ЧТЕНИЕ. overlock_thread_count и pressing сняты
// парком оборудования; их места остались занятыми теми значениями, в которые маршалился NULL, ровно
// чтобы карточка, у которой их не было, хешировалась как раньше. Здесь проверяется вторая половина:
// проекция их БОЛЬШЕ НЕ ЧИТАЕТ.
//
// Карточка, у которой они БЫЛИ заполнены, меняет отпечаток относительно того, что подписали до
// миграции, — и это честно: 0306 перенесла её pressing в notes и завела ей профиль оверлока, то есть
// подписанное содержание действительно двинулось.
//
// T6 удаляет оба поля из entity — этот тест уйдёт вместе с ними, караулить заморозку останется
// эталонный hex выше.
func TestRetiredConstructionFieldsAreFrozenNotRead(t *testing.T) {
	tc := machineShapeConstructionCard()
	tc.Construction.Pressing = ns("отпарить на готовом")
	tc.Construction.OverlockThreadCount = ni32(4)
	require.Equal(t, machineCompatConstructionDigest, constructionDigest(tc),
		"снятые поля всё ещё хешируются: их значения обязаны быть заморожены константами")
}
