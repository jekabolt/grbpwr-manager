...
func TestMarkerLayoutFactsFromBlob(t *testing.T) {
	t.Run("reads the two forbidden properties", func(t *testing.T) {
		cases := map[string]entity.MarkerLayoutFacts{
			...
			`{"schemaVersion":1,"placements":[{"rotDeg":180}]}`:                  {SchemaVersion: 1, HasHalfTurn: true},
			`{"schemaVersion":3,"placements":[{"flipped":true}]}`:                {SchemaVersion: 3, HasFlip: true},
			`{"schemaVersion":3,"placements":[{"rotDeg":180},{"flipped":true}]}`: {SchemaVersion: 3, HasHalfTurn: true, HasFlip: true},
