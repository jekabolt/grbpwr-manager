...
// total in base currency, a per-kind breakdown, and the amortized unit_cost_with_dev = production
// unit cost + dev_total / Σ order_qty. The amortized figure is set only when the size run and a
// base-currency production unit cost are both known; development stays a PERIOD cost and is never
// seeded into cost_price, so this is purely informational. has_unconverted flags a partial total
// (some row's currency had no FX rate).
func ComputeTechCardDevCostSummary(card *entity.TechCard, expenses []entity.TechCardDevExpense, fx CostingFx) *pb_common.TechCardDevCostSummary {
