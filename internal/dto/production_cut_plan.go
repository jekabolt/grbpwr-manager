...
// product_id (+размер), ЛИБО output_variant_id (chk_prl_variant_xor), поэтому ровно одно из полей
// ключа ненулевое, а вместе они дают устойчивую идентичность колонки в обоих мирах.
...
func (g *cutPlanGroup) resolve(piece *entity.TechCardPiece, items []entity.TechCardBomItem) (cutPlanArticle, string) {
	// (а) РЕЦЕПТ НАЗЫВАЕТ ДЕТАЛЬ. Единственный случай, где наряд ничего не выводит: строка рецепта
	// сама сказала, какой слот идёт на эту деталь. FK сначала, ULID вторым — тот же порядок
