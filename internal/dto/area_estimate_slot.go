...
func slotAreaEstimate(
	...
	//
	// КОЭФФИЦИЕНТ РАСКРОЯ ЗДЕСЬ ТОЖЕ НЕ НАЧИСЛЯЕТСЯ, И ЭТО ОСОЗНАННО, А НЕ ЗАБЫТО (W3). С W3
	// коэффициент входит в деньги АВТОРСКОЙ нормы обоими путями — и marker, и netto/manual/dxf, —
	// так что соблазн «для единообразия» дописать его и сюда очевиден. Отказ держится на том, чем
	// ступень 0 ОБЪЯВЛЕНА: она несёт НОЛЬ гросс-апов, и это её единственное внятное свойство —
	// «столько ткани в изделии, и ни сантиметра сверх». Начислить один множитель из двух значит
	// получить число, которое уже не нижняя граница по построению и ещё не норма, и про которое
	// нельзя сказать одной фразой, чего в нём нет. Разрыв с dxf-нормой того же слота при этом
	// остаётся ЧЕСТНЫМ: применение нормы поднимает деньги на (1 + процент) × коэффициент, то есть
	// ровно на два множителя, которые оператор в этот момент объявляет.
	//
	// Цена этого решения названа там же, где она платится: hasEstimate у colorwayCost запрещает
	// такой цифре сеять product.cost_price и стоять под релизом.
...
//
// ЭТО ССЫЛКА, А НЕ ВТОРОЙ СПИСОК, и переписывать её обратно в switch нельзя. Список рулонных секций
// живёт в entity/roll_goods.go, и его собственная шапка называет копию в internal/dto именно тем
// дрейфом, ради запрета которого список туда и переехал: пятая семья, добавленная в один экземпляр,
// оставила бы второй при четырёх — молча. Здесь же с W3 проходит ГРАНИЦА КОЭФФИЦИЕНТА РАСКРОЯ
// (EffectiveCuttingCoefficient отвечает по тому же предикату), так что разъехавшиеся копии означали
// бы не косметику, а деньги: одна сторона начислила бы рулонную надбавку там, где другая её не ждёт.
func isRollGoodsSection(s entity.TechCardBomSection) bool { return entity.IsRollGoodsSection(s) }
