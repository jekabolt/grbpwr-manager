...
func TestTechCardMarkerSummaryToPbDerivesConsumption(t *testing.T) {
	...
	// The old `if Sets <= 0 { return UsedLengthCm }` guard is gone: an unreadable row must not report
	// the whole spread as one garment's norm.
	broken := m
	broken.Composition, broken.TotalUnits, broken.Sets, broken.SizeId =
		nil, sql.NullInt64{}, sql.NullInt64{}, sql.NullInt64{}
	require.Equal(t, "512.4", TechCardMarkerSummaryToPb(broken).ConsumptionPerUnitCm.Value)
	require.Empty(t, TechCardMarkerSummaryToPb(broken).Composition)
}
