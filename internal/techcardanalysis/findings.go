...
type Finding struct {
	...
	// Money — находка ЦИТИРУЕТ КОНФИДЕНЦИАЛЬНУЮ ДЕНЕЖНУЮ ВЕЛИЧИНУ: закупочную цену, валюту линии
	// или их соотношение. Такая находка НЕ ОТДАЁТСЯ вызывающему без costing:read.
	//
	// ПОЧЕМУ ПРИЗНАК ЖИВЁТ ЗДЕСЬ, А НЕ СПИСКОМ ИМЁН В ОБРАБОТЧИКЕ. Обработчик со списком «денежные
	// проверки — B5а, B5б, B5в» — это место, куда НОВАЯ денежная проверка не попадёт, и утечёт она
	// по умолчанию, молча. Признак рядом с проверкой ошибается в другую сторону: забытый флаг
	// прячет не деньги, а находку, и это видно.
	//
	// ГРАНИЦА, ПО КОТОРОЙ ОН СТАВИТСЯ, — ТА ЖЕ, ПО КОТОРОЙ РЕЖЕТ stripTechCardCosting
	// (internal/apisrv/admin/costing_rbac.go): «структура остаётся, деньги уходят». Величина, валюта
	// и отношение величин — деньги. ИМЯ НЕДОСТАЮЩЕГО ФАКТА — НЕ ДЕНЬГИ: тот же файл оставляет
	// `price_source` и причину `no_price` осознанно, и по этому же правилу «cmt_cost не задан»
	// (B6) деньгами не является — эта самая фраза уже едет без costing:read оговоркой сметы
	// (stripStyleCostEstimate её не снимает).
	//
	// НА ПРОВОД НЕ ЕДЕТ: это входные данные редактирования, а не факт о карточке.
	Money bool
