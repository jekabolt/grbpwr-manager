...
func (s *Service) ServeFile(w http.ResponseWriter, r *http.Request) {
	...
	case row.RevokedAt.Valid:
		// ВТОРОЙ РУБИЛЬНИК, НЕЗАВИСИМЫЙ ОТ УРОВНЯ. Панель ставит этот штамп при уходе с `link`
		// (там он дублирует проверку выше и служит читаемым состоянием), но проверка нужна не ради
		// панели: это единственный способ погасить ОДНУ ссылку прямо в базе, не трогая уровень
		// файла и не объясняя людям, почему файл вдруг закрылся. Снимает штамп только возврат на
		// уровень — «пересоздать» его переживает намеренно.
		notFound("revoked")
		return
