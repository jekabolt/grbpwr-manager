...
func (s *Server) CreateProductionRun(ctx context.Context, req *pb_admin.CreateProductionRunRequest) (*pb_admin.CreateProductionRunResponse, error) {
	...
	if ins.Status != entity.ProductionRunDraft && ins.Status != entity.ProductionRunPlanned && ins.Status != entity.ProductionRunInProgress {
		return nil, status.Error(codes.InvalidArgument, "a production run is created as draft, planned or in_progress; received/closed/cancelled are reached through their flows")
	}
	...
	// be refused.
	// ЧЕРНОВИК ГЕЙТ НЕ ПРОХОДИТ (Ф2). Гейт отвечает на вопрос «можно ли это резать», а черновик резать
	// никто не собирается: он существует, чтобы посчитать деньги по конкретному составу. Заставить
	// его отвечать на чужой вопрос значило бы запретить прикидку по карточке, у которой ещё нет
	// раскладок, — то есть ровно по той, ради которой прикидку и делают.
	//
	// Гейт применяется на переходе draft → planned, там же, где появляется резерв.
	...
	//
	// ЧЕРНОВИК НЕ РЕЗЕРВИРУЕТ. Прикидка «сколько будет стоить сто чёрных и полсотни белых» не имеет
	// права занимать ткань: соседний прогон увидел бы нехватку из-за чужого черновика, а узнал бы об
	// этом тот, кто пришёл на склад вторым. Резерв навешивает переход draft → planned.
	if ins.Status != entity.ProductionRunDraft {
...
func (s *Server) UpdateProductionRun(ctx context.Context, req *pb_admin.UpdateProductionRunRequest) (*pb_admin.UpdateProductionRunResponse, error) {
	...
	// ПЕРЕХОД draft → всё остальное: здесь черновик становится обязательством, и здесь же появляется
	// резерв ткани. Гейт готовности на этом переходе НЕ применяется намеренно: он живёт на создании
	// (runReadinessCreateGate) и на старте кроя, а промежуточное «партию решили шить» не должно
...
// ЛИНИИ ПРОГОНА — ЧАСТЬ ФОРМУЛЫ, А НЕ ЕЁ ВЫЗЫВАЮЩЕГО. Живая ветка ценится по СОБСТВЕННОМУ миксу
// размеров партии, а не по стандартной себестоимости стиля: партия из одних XL планировалась
...
func (s *Server) plannedUnitCostFor(ctx context.Context, releaseId sql.NullInt64, techCardId int, actualWastagePercent decimal.NullDecimal, lines []entity.ProductionRunLine) (decimal.NullDecimal, string, error) {
	...
	// From the live card, priced on THIS run's own size mix (lines) rather than on the style's
	// standard cost, and using the run's ACTUAL cutting wastage when set (it overrides every BOM
...
func (s *Server) snapshotPlannedCost(ctx context.Context, ins *entity.ProductionRunInsert) error {
	unit, currency, err := s.plannedUnitCostFor(ctx, ins.ReleaseId, ins.TechCardId, ins.ActualWastagePercent, ins.Lines)
	if err != nil {
		return err
	}
...
func (s *Server) plannedUnitCostToday(ctx context.Context, run *entity.ProductionRun) *pb_decimal.Decimal {
	...
	// run.Lines, not the card: «сегодня» обязано спрашивать ровно то же, что спрашивал снапшот, иначе
	// бейдж «карточка изменилась» загорится на прогоне, у которого изменился только микс — а он у
	// сохранённого прогона не меняется сам по себе.
	unit, currency, err := s.plannedUnitCostFor(ctx, run.ReleaseId, run.TechCardId, run.ActualWastagePercent, run.Lines)
	if err != nil {
