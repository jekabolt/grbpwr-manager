...
var (
	// ⚠ ПРОЗАИЧЕСКАЯ БАЗА ОСТАЁТСЯ ЛИТЕРАЛОМ, ПОТОМУ ЧТО ВЫВОДИТЬ ЕЁ НЕ ИЗ ЧЕГО: прозаическая ветка
	// НЕ СТАВИТ ПОТОЛКА ВОВСЕ (design_run.go: maxTokens = 0), то есть у её ответа нет числа, которое
	// можно было бы умножить на тариф. 0.03 здесь — прикидка порядка величины, и она честно названа
	// прикидкой, а не границей.
