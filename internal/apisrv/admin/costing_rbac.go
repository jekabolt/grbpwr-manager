...
func stripTechCardCosting(tc *pb_common.TechCard) {
	...
	for _, c := range tc.Colorways {
		...
		c.Prices = nil
		c.NetPrices = nil
		// AreaEstimates (Ф1) НАМЕРЕННО ОСТАЮТСЯ. Это расход ткани на изделие — метры, не деньги, — и
		// правило этого файла ровно одно: «структура остаётся, деньги уходят». Пер-изделийный расход
		// уже остаётся у строки рецепта тремя строчками выше (stripTechCardColorwayUsageCosting
		// снимает только line_total/size_run_total) и у сметы (stripStyleCostEstimate снимает цены и
		// оставляет consumption). Вырезав его здесь, мы получили бы аккаунт, который у слота с
		// авторской нормой видит расход, а у соседнего, посчитанного оценкой, — пустоту: одна и та же
		// ткань в одном списке то есть, то нет, по причине, которую на экране не объяснить.
		//
		// Сообщение спроектировано так, чтобы это решение было безопасным: в нём нет ни цены, ни
		// суммы, ни валюты. Причина отказа `no_price` — не сумма, а имя недостающего факта, того же
		// класса, что провенансная метка price_source, которую stripStyleCostEstimate оставляет
		// осознанно. Деньги этой оценки живут в TechCardCosting, и он вырезается целиком выше.
		//
		// ShadowAreaEstimates ОСТАЮТСЯ ПО ТОМУ ЖЕ ДОВОДУ И БЕЗ ЕДИНОЙ ОГОВОРКИ: это то же сообщение
		// того же типа с тем же содержимым — метры на изделие по геометрии, — только для слота, у
		// которого норма уже вписана. Оно не несёт ни цены, ни суммы, ни валюты, а сравнивают его с
		// нормой из строки рецепта, которую этот же аккаунт и так видит.
	}
}
