...
// goldenTwoClothAuthority — the ONE paragraph a several-cloths run shares with a single-cloth one,
// captured from the composer and pasted here as a literal.
//
// ⚠ WHY THIS EXISTS AND WHY IT IS NOT A WHOLE-PROMPT GOLDEN. The several-cloths wave left the
// composed several-cloths prompt uncovered end to end: every test above asserts with Contains, so a
// change anywhere in that text passes in silence. H-13 walked straight into the hole — its edit to
// the order of authority moves EXACTLY ONE of the twelve paragraphs a two-cloth run composes (this
// one; the cloth list, the layout and the four closing paragraphs are byte for byte what they
// were), and not one assertion in the package could see it.
//
// A whole-prompt golden would have covered it and more, and it was deliberately not written here: it
// would freeze the cloth-list wording, which this wave neither wrote nor can argue for, and the
// wave that did write it declined to freeze it. What is pinned is what this wave moves — the
// paragraph the single-cloth goldens already pin — so the two can never drift apart unnoticed.
const goldenTwoClothAuthority = "Fabric. The cloth of this garment is stated in more than one way at once, and the statements may disagree. Resolve every disagreement in this fixed order of authority, the same way on every run:" +
	"\n" + "1. THE FABRIC PHOTOGRAPH (image 4) governs the MATERIAL of this garment: weave or knit structure, surface texture, pile, sheen, transparency, weight and the way the cloth drapes and folds. Read the cloth from that image and from nothing else." +
	"\n" + "2. THE STATED COLOUR — the `colour` block above — governs the COLOUR of this garment, and OVERRIDES the colour of the fabric photograph. Where the two disagree, keep the photograph's material and re-colour it: the garment is the stated colour even when the swatch photograph is another one." +
	"\n" + "3. THE FABRIC DESCRIPTION IN WORDS — the `fabric in words` block above — adds only what neither the photograph nor the stated colour already says. It never overrides either of them: a word that contradicts the photograph's material or the stated colour is to be ignored, and where a fabric photograph is ranked above this clause it states the transparency, the weight and the drape as surely as it states the weave, so against a photograph a word about any of those is description and not instruction. WHEN THIS RUN CARRIES NO FABRIC PHOTOGRAPH, nothing ranked above this clause states the material at all: a stated colour states COLOUR and nothing else — not how sheer or opaque the cloth is, not its weight, not its hand, not its drape — so a word about any of those adds rather than contradicts, and the weave or knit, the weight, the surface and the drape of this garment are to be built from those words."

// TestSeveralClothsCarryTheSameAuthorityParagraphAsOneCloth — the order of precedence is ONE text
// on both paths, and renderFabricSection appends the very same paragraph after the cloth list.
//
// Held word for word rather than by Contains, because the failure this guards is a SOFTENING: a
// clause reworded on one path and not the other would leave a two-cloth run arbitrating its cloth
// by a different rule than a one-cloth run, and nothing else in the package would notice.
func TestSeveralClothsCarryTheSameAuthorityParagraphAsOneCloth(t *testing.T) {
	...
	require.Equal(t, goldenTwoClothAuthority, authority,
		"the order of authority a several-cloths run states must be the frozen one, word for word")

	// …and it really is the SAME text the single-cloth goldens carry, modulo the image number the
	// swatch happens to have. Two copies of one rule that drift are worse than one copy.
	require.Contains(t, goldenAllThree,
		strings.Replace(goldenTwoClothAuthority, "(image 4)", "(image 3)", 1),
		"the several-cloths paragraph and the single-cloth golden are one rule, not two")
}
