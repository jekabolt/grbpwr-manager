...
// ВОЗОБНОВЛЕНИЕ ОПЛАЧЕННОГО ЗАДАНИЯ НЕ ПЕРЕПИСЫВАЕТ ИСТОРИЮ ПРОМПТА.
//
// Запись промпта стояла ВЫШЕ поиска принятой попытки, и на возобновлении это било дважды.
// Во-первых, колонка получала заново собранный текст, который поставщику НЕ отправлялся ни разу:
// состав входов мог измениться между проходами (удалили медиа, переехал сборщик), и история
// начинала утверждать про потраченные деньги неправду. Во-вторых, отказ этой записи обрывал проход
// ДО бесплатного сбора уже оплаченного результата — дорогая ошибка ради дешёвой записи.
//
// Проба утверждает отсутствие события, и потому рядом стоит ПОЛОЖИТЕЛЬНЫЙ КОНТРОЛЬ: тот же проход
// обязан дойти до сбора. Без него «записи не было» зеленело бы и на проходе, который не делал
// вообще ничего.
func TestResumingAPaidTaskDoesNotRewriteThePromptHistory(t *testing.T) {
	prior := acceptedThreedRun()
	st := &fakeStore{getRun: &prior}
	w := chargedThreedWorker(t, st, 30)
	...
	require.NotContains(t, st.events, "record_prompt",
		"возобновление ничего не отправляет — значит и переписывать текст ему нечем: %v", st.events)
	require.Empty(t, st.recordedPrompts, "в колонке обязан остаться текст, который реально ушёл")
	require.Len(t, st.finished, 1,
		"положительный контроль: проход дошёл до СБОРА, то есть проба меряет живой путь")
}
