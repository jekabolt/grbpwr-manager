...
// ReadStore implements dependency.BQCacheStoreRead.
type ReadStore struct {
	storeutil.Base
}

func (s *ReadStore) SumBQFunnelAnalysis(ctx context.Context, from, to time.Time) (*entity.FunnelAggregate, error) {
	query := `
		SELECT
	...
	`
	agg, err := storeutil.QueryNamedOne[entity.FunnelAggregate](ctx, s.DB, query, map[string]any{
...
func (s *ReadStore) GetDailyBQFunnelAnalysis(ctx context.Context, from, to time.Time) ([]entity.DailyFunnel, error) {
	query := `
...
func (s *ReadStore) GetBQOOSImpact(ctx context.Context, from, to time.Time, limit, offset int) ([]entity.OOSImpactMetric, error) {
...
func (s *ReadStore) GetBQPaymentFailures(ctx context.Context, from, to time.Time) ([]entity.PaymentFailureMetric, error) {
	query := `
...
func (s *ReadStore) GetBQWebVitals(ctx context.Context, from, to time.Time) ([]entity.WebVitalMetric, error) {
	query := `
...
func (s *ReadStore) GetBQUserJourneys(ctx context.Context, from, to time.Time, limit, offset int) ([]entity.UserJourneyMetric, error) {
	page := storeutil.BQPageParams{Limit: limit, Offset: offset}
	query := `
...
func (s *ReadStore) GetBQSessionDuration(ctx context.Context, from, to time.Time) ([]entity.SessionDurationMetric, error) {
	query := `
...
func (s *ReadStore) GetBQSizeIntent(ctx context.Context, from, to time.Time, limit, offset int) ([]bq.SizeIntentRow, error) {
	page := storeutil.BQPageParams{Limit: limit, Offset: offset}
	query := `
...
func (s *ReadStore) GetBQDeviceFunnel(ctx context.Context, from, to time.Time) ([]entity.DeviceFunnelMetric, error) {
	query := `
...
func (s *ReadStore) GetBQFormErrors(ctx context.Context, from, to time.Time, limit, offset int) ([]entity.FormErrorMetric, error) {
	page := storeutil.BQPageParams{Limit: limit, Offset: offset}
	query := `
...
func (s *ReadStore) GetBQExceptions(ctx context.Context, from, to time.Time, limit, offset int) ([]entity.ExceptionMetric, error) {
	page := storeutil.BQPageParams{Limit: limit, Offset: offset}
	query := `
...
func (s *ReadStore) GetBQNotFoundPages(ctx context.Context, from, to time.Time, limit, offset int) ([]entity.NotFoundMetric, error) {
	page := storeutil.BQPageParams{Limit: limit, Offset: offset}
	query := `
...
func (s *ReadStore) GetBQHeroFunnel(ctx context.Context, from, to time.Time) ([]entity.HeroFunnelMetric, error) {
	query := `
...
func (s *ReadStore) GetBQPaymentRecovery(ctx context.Context, from, to time.Time) ([]entity.PaymentRecoveryMetric, error) {
	query := `
...
func (s *ReadStore) GetBQCheckoutTimings(ctx context.Context, from, to time.Time) ([]entity.CheckoutTimingMetric, error) {
	query := `
...
func (s *ReadStore) GetBQTimeOnPage(ctx context.Context, from, to time.Time, limit, offset int) ([]entity.TimeOnPageRow, error) {
	...
	for _, r := range rows {
		...
		result = append(result, entity.TimeOnPageRow{
			Date: date, PagePath: r.PagePath, AvgVisibleTimeSeconds: r.AvgVisibleTimeSeconds,
			AvgTotalTimeSeconds: r.AvgTotalTimeSeconds, AvgEngagementScore: r.AvgEngagementScore, PageViews: r.PageViews,
		})
	}
	return result, nil
}

func (s *ReadStore) GetBQProductZoom(ctx context.Context, from, to time.Time, limit, offset int) ([]entity.ProductZoomRow, error) {
	page := storeutil.BQPageParams{Limit: limit, Offset: offset}
	query := `
	...
	for _, r := range rows {
		...
		result = append(result, entity.ProductZoomRow{
			Date: date, ProductID: r.ProductID, ProductName: r.ProductName, ZoomMethod: r.ZoomMethod, ZoomCount: r.ZoomCount,
		})
	}
	return result, nil
}

func (s *ReadStore) GetBQImageSwipes(ctx context.Context, from, to time.Time, limit, offset int) ([]entity.ImageSwipeRow, error) {
	page := storeutil.BQPageParams{Limit: limit, Offset: offset}
	query := `
	...
	for _, r := range rows {
		...
		result = append(result, entity.ImageSwipeRow{
			Date: date, ProductID: r.ProductID, ProductName: r.ProductName, SwipeDirection: r.SwipeDirection, SwipeCount: r.SwipeCount,
		})
	}
	return result, nil
}

func (s *ReadStore) GetBQSizeGuideClicks(ctx context.Context, from, to time.Time, limit, offset int) ([]entity.SizeGuideClickRow, error) {
	page := storeutil.BQPageParams{Limit: limit, Offset: offset}
	query := `
	...
	for _, r := range rows {
		...
		result = append(result, entity.SizeGuideClickRow{
			Date: date, ProductID: r.ProductID, ProductName: r.ProductName, PageLocation: r.PageLocation, ClickCount: r.ClickCount,
		})
	}
	return result, nil
}

func (s *ReadStore) GetBQDetailsExpansion(ctx context.Context, from, to time.Time, limit, offset int) ([]entity.DetailsExpansionRow, error) {
	page := storeutil.BQPageParams{Limit: limit, Offset: offset}
	query := `
	...
	for _, r := range rows {
		...
		result = append(result, entity.DetailsExpansionRow{
			Date: date, ProductID: r.ProductID, ProductName: r.ProductName, SectionName: r.SectionName, ExpandCount: r.ExpandCount,
		})
	}
	return result, nil
}

func (s *ReadStore) GetBQNotifyMeIntent(ctx context.Context, from, to time.Time, limit, offset int) ([]entity.NotifyMeIntentRow, error) {
	page := storeutil.BQPageParams{Limit: limit, Offset: offset}
	query := `
	...
	for _, r := range rows {
		...
		result = append(result, entity.NotifyMeIntentRow{
			Date: date, ProductID: r.ProductID, ProductName: r.ProductName, Action: r.Action, Count: r.Count, ConversionRate: r.ConversionRate,
		})
	}
	return result, nil
}

func (s *ReadStore) GetBQAddToCartRate(ctx context.Context, from, to time.Time, granularity entity.TrendGranularity, limit, offset int) (*entity.AddToCartRateAnalysis, error) {
	page := storeutil.BQPageParams{Limit: limit, Offset: offset}
	...
	params := map[string]any{"fromDate": from.Format("2006-01-02"), "toDate": to.Format("2006-01-02"), "limit": page.EffectiveLimit(), "offset": page.EffectiveOffset()}
	productRows, err := storeutil.QueryListNamed[productRow](ctx, s.DB, productQuery, params)
	if err != nil {
	...
	for _, r := range productRows {
		products = append(products, entity.AddToCartRateProductRow{
			ProductID: r.ProductID, ProductName: r.ProductName, ViewCount: r.ViewCount,
			AddToCartCount: r.AddToCartCount, CartRate: r.CartRate,
		})
	...
	trendRows, err := storeutil.QueryListNamed[trendRow](ctx, s.DB, trendQuery, trendParams)
	if err != nil {
	...
	for _, r := range trendRows {
		date, err := storeutil.ParseDateStr(r.TrendDate)
		if err != nil {
			return nil, fmt.Errorf("parse trend date: %w", err)
		}
		globalTrend = append(globalTrend, entity.AddToCartRateGlobalRow{
			Date: date, TotalViews: r.TotalViews, TotalAddToCarts: r.TotalAddToCarts, GlobalCartRate: r.GlobalCartRate,
		})
	}

	return &entity.AddToCartRateAnalysis{
		Products: products, GlobalTrend: globalTrend, AvgViewCount: avgViewCount, AvgCartRate: avgCartRate,
	}, nil
}

func (s *ReadStore) GetBQBrowserBreakdown(ctx context.Context, from, to time.Time, limit, offset int) ([]entity.BrowserBreakdownRow, error) {
	page := storeutil.BQPageParams{Limit: limit, Offset: offset}
	query := `
...
func (s *ReadStore) GetBQNewsletter(ctx context.Context, from, to time.Time) ([]entity.NewsletterMetricRow, error) {
	query := `
...
func (s *ReadStore) GetBQAbandonedCart(ctx context.Context, from, to time.Time) ([]entity.AbandonedCartRow, error) {
	query := `
