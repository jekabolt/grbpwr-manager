...
func (s *Store) AddTechCardDevExpense(ctx context.Context, e entity.TechCardDevExpense) (entity.TechCardDevExpense, error) {
	id, err := storeutil.ExecNamedLastId(ctx, s.DB, `
		INSERT INTO tech_card_dev_expense
			(tech_card_id, kind, description, amount, currency, amount_base, fitting_id, incurred_at)
		VALUES (:tech_card_id, :kind, :description, :amount, :currency, :amount_base, :fitting_id, :incurred_at)`,
		map[string]any{
	...
	row, err := storeutil.QueryNamedOne[entity.TechCardDevExpense](ctx, s.DB, `
		SELECT id, tech_card_id, kind, description, amount, currency, amount_base, fitting_id, incurred_at, created_at
