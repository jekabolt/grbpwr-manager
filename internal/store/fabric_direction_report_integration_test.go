...
func TestFabricDirectionGapsReport(t *testing.T) {
	...
	// Since Ф2 the СОСТАВ is not optional either, and for the same shape of reason: an insert with
	// no состав is refused before the transaction opens, because an empty one means total_units 0 —
	// a zero divisor for every costing read downstream. markerSizing fills the ЛЕГАСИ pair AND the
	// one-entry состав dto derives from it, so this fixture describes one раскладка rather than half.
	d := func(v string) decimal.Decimal { return decimal.RequireFromString(v) }
	gapMarker := entity.TechCardMarkerInsert{
		...
		UsedLengthCm: d("100"), PlacedCount: 2, TotalCount: 2,
		Layout:      `{"schemaVersion":1,"pieces":[],"placements":[]}`,
		LayoutFacts: entity.MarkerLayoutFacts{SchemaVersion: 1},
	}
	markerSizing(&gapMarker, szA, 1)
	_, err = T.SaveMarker(ctx, gapID, 0, gapMarker, "tester")
	require.NoError(t, err)
