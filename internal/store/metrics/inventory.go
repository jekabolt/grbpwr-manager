	...
	query := fmt.Sprintf(`
		WITH sold AS (
			SELECT
				...
				COALESCE(SUM(CASE WHEN oi.cost_price_at_sale IS NOT NULL AND COALESCE(oi.product_price_base, pp_base.price) IS NOT NULL THEN oi.cost_price_at_sale * oi.quantity ELSE 0 END), 0) AS sold_cost,
				COALESCE(SUM(CASE WHEN oi.cost_price_at_sale IS NOT NULL AND COALESCE(oi.product_price_base, pp_base.price) IS NOT NULL THEN COALESCE(oi.product_price_base, pp_base.price) * oi.quantity ELSE 0 END), 0) AS costed_revenue
			FROM order_item oi
			JOIN customer_order co ON oi.order_id = co.id
		...
		SELECT
			...
			COALESCE(SUM(s.sold_cost), 0) AS revenue_cost,
			COALESCE(SUM(s.costed_revenue), 0) AS costed_revenue,
			CASE
