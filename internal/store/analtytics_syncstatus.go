...
type analyticsSyncStatusStore struct {
	*MYSQLStore
}

// SyncStatus returns an object implementing the SyncStatusStore interface.
func (ms *MYSQLStore) SyncStatus() dependency.SyncStatusStore {
	return &analyticsSyncStatusStore{MYSQLStore: ms}
}

// UpdateGA4SyncStatus updates the sync status for a given sync type.
func (s *analyticsSyncStatusStore) UpdateGA4SyncStatus(ctx context.Context, syncType string, lastSyncDate time.Time, success bool, recordsSynced int, errorMsg string) error {
	query := `
	...
	return ExecNamed(ctx, s.DB(), query, params)
}

// GetGA4LastSyncDate returns the last successful sync date for a given sync type.
func (s *analyticsSyncStatusStore) GetGA4LastSyncDate(ctx context.Context, syncType string) (time.Time, error) {
	query := `
	...
	r, err := QueryNamedOne[row](ctx, s.DB(), query, map[string]any{"syncType": syncType})
	if err != nil {
	...
	return parseDateStr(r.LastSyncDate)
}

// GetGA4MinLastSyncDate returns the minimum last successful sync date across all sync types.
// Used to ensure we never skip dates when one sync (e.g. daily_metrics) succeeds but others
// (e.g. BQ syncs) fail—the next run will retry from the oldest successful point.
func (s *analyticsSyncStatusStore) GetGA4MinLastSyncDate(ctx context.Context) (time.Time, error) {
	query := `
	...
	r, err := QueryNamedOne[row](ctx, s.DB(), query, nil)
	if err != nil {
		return time.Time{}, err
	}
	...
	return parseDateStr(r.MinDate.String)
}

// GetAllSyncStatuses returns the latest sync status for every sync_type.
func (s *analyticsSyncStatusStore) GetAllSyncStatuses(ctx context.Context) ([]entity.SyncSourceStatus, error) {
	query := `
...
func (s *analyticsSyncStatusStore) DeleteOldAnalyticsData(ctx context.Context, olderThan time.Time) (int64, error) {
	tables := []string{
		// GA4 tables
		"ga4_daily_metrics",
