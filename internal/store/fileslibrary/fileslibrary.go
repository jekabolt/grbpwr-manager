...
func (s *Store) MergeTopics(ctx context.Context, sourceID, targetID int) (int, error) {
	...
	err = s.txFunc(ctx, func(ctx context.Context, rep dependency.Repository) error {
		...
		//
		// ЦЕНА IGNORE НАЗВАНА ВСЛУХ: он глушит НЕ ТОЛЬКО 1062, но и 1452. На этом единственном
		// пути составной ключ перестаёт быть последним рубежом и превращается в молчаливый
		// пропуск строки — файл просто не переедет. Убрать IGNORE нельзя (без него слияние падало
		// бы о законное столкновение «файл уже лежит в цели»), поэтому корректность здесь держат
		// ДВЕ другие вещи: соединение по ИМЕНИ выше, дающее роль цели для каждой роли источника, и
		// тесты, которые проверяют СОСТАВ пар (файл, имя роли) в цели, а не код возврата. Проба,
		// утверждающая только «слияние прошло», на этом пути не измеряет ничего.
		if _, err := storeutil.ExecNamedRows(ctx, rep.DB(), `
