...
func (s *Store) SaveNoteContent(ctx context.Context, in entity.LibraryNoteSave) (*entity.LibraryNoteSaveResult, error) {
	...
	err = s.txFunc(ctx, func(ctx context.Context, rep dependency.Repository) error {
		...
		// Число затронутых строк здесь НЕ показатель: сохранение того же текста тем же человеком в
		// ту же секунду не меняет ни одной колонки, и MySQL честно отвечает «0». Строку мы уже
		// прочитали и держим монопольно, так что «её нет» тут невозможно.
