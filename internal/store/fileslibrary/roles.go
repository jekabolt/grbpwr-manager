...
//
// РАЗРЕШЕНИЕ МОЖЕТ НЕ НАЙТИ НИЧЕГО, И ЭТО НЕ ОШИБКА, А ТРЕТИЙ СЛУЧАЙ. Адрес, сохранённый ДО
// переноса 0323, несёт id ГЛОБАЛЬНОЙ роли; клоны получили НОВЫЕ id, а исходная строка осталась
// лежать мёртвой (владельца нет, ни одна строка связи в неё не смотрит). Такой id в этом ответе
// либо отсутствует вовсе, либо приезжает с нулевым владельцем — и в обоих случаях клиент обязан
// СНЯТЬ `frole` с адреса, а не фильтровать по нему: фильтр по мёртвому id молча отдаёт пустую
// выдачу, то есть «в этой роли ничего нет» вместо «этой роли больше нет».
//
...
func (s *Store) SetFileRoles(ctx context.Context, fileIDs []int, projectTopicID, roleID int) (int, error) {
	...
	err = s.txFunc(ctx, func(ctx context.Context, rep dependency.Repository) error {
		...
		if roleID > 0 {
			...
			// РОЛЬ ОБЯЗАНА БЫТЬ РОЛЬЮ ЭТОГО ЖЕ ПРОЕКТА (0323). Мёртвая строка переноса (владельца
			// нет) отвергается этим же условием и той же фразой — «принадлежит другому проекту»
			// для неё чуть неточно, но заводить второй текст ради состояния, недостижимого ни
			// одним экраном, значило бы платить словами за строку, которую никто не выберет.
			//
			// Проверка стоит ПЕРВОЙ из двух:
