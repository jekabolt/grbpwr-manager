...
// RecordRunPrompt пишет в строку прогона ТОТ ТЕКСТ, который воркер через мгновение отправит
// поставщику ЭТИМ маршрутом.
//
...
// буквально на одновызовном флэтовом маршруте; на `per_view` каждому платному вызову дописывается
// свой «view:» — надстройка над той же базой. 3D же не режет промпт и никогда его не слал: у
// поворотного стола единственный текст — короткая подсказка о ПОВЕРХНОСТИ (`texture_prompt`), а у
// части моделей текстового поля нет вовсе, и тогда в колонке ПУСТО, потому что пусто и ушло. Текст
// спрашивают у самого маршрута (designgen.PromptCarrier), а не выводят из рода прогона.
...
//
// ⚠ У ЭТОГО ВЫБОРА ЕСТЬ ВТОРОЙ ИСТОЧНИК ДРЕЙФА, И ОН ШИРЕ ПЕРВОГО: СМЕНА САМОГО СБОРЩИКА. Абзац
// выше называет живой резолв медиа, то есть РАЗНЫЕ ВХОДЫ при одном бинаре. Но между попытками
// ложится и деплой, и тогда при тех же входах текст собирает ДРУГОЙ composePrompt. Путь измерен, а
// не заподозрен: ретраебельный провал возвращает строку в `pending` (FailRun, до
// designMaxPaidAttempts=5 оплаченных и designMaxRounds=10 кругов), следующий круг снова зовёт
// `buildJob` и — на маршруте картинок, который Collector НЕ реализует, поэтому `pendingID` там
// всегда пуст, — снова пишет колонку. У `design_run_attempt` колонки с текстом нет вовсе, так что
// прежний текст после этого не восстановить ниоткуда.
//
// ОТВЕТ ТОТ ЖЕ, И ПРИЧИНА ТА ЖЕ, ЧТО ВЫШЕ: картинку, которая у прогона в итоге есть, сделал
// ПОСЛЕДНИЙ текст. Заморозить первый значило бы оставить в колонке слова отправки, не давшей
// ничего, — ровно тот отказ, который уже чинили на 3D («приписанные деньгам слова хуже пустой
// колонки»), только теперь врал бы не род прогона, а номер попытки.
//
// ЧЕСТНАЯ РАЗНИЦА — НЕ В НАПРАВЛЕНИИ, А В РАДИУСЕ. Удалённая картинка задевает один прогон и
// редко; деплой задевает СРАЗУ ВСЕ ретраебельные строки, висящие в `pending`. Это довод за
// ПОПЫТОЧНОЕ хранение текста (колонка на `design_run_attempt`, отдельная миграция и своя волна), а
// не за заморозку первой отправки. Дверь названа здесь именно поэтому: чтобы следующая правка
// промпта не открывала её заново как новость.
