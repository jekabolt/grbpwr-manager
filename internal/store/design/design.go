...
const (
	...
	// MaxBandJournal — the band ships at most this many journal lines, newest first.
	// WHAT HAPPENS AFTER THE FIFTIETH, since the plan does not say: they are DROPPED from the
	// band, and nothing is lost by it — GetSheetVersion returns a version's own journal in full,
	// oldest first. The band's journal is a recency strip, not the record.
	MaxBandJournal = 50
