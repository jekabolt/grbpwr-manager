		...
		if req.ForInput {
			...
			for _, f := range req.Frames {
				...
				ON DUPLICATE KEY UPDATE
					...
					-- как была.
					--
					-- ⚠ И НИ ОДНОГО ДВОЕТОЧИЯ В ЭТИХ КОММЕНТАРИЯХ. sqlx разбирает текст ДО MySQL и
					-- читает «двоеточие плюс пробел» как параметр с ПУСТЫМ именем; запрос падает на
					-- связывании словами «could not find name», то есть не там, где станешь искать.
					-- Здесь это уже случилось однажды, и поймала это соседняя проба, а не сборка.
					--
					-- ⚠ ЗДЕСЬ СТОЯЛО БЕЗУСЛОВНОЕ ОБНУЛЕНИЕ, И ЭТО БЫЛО МОЛЧАЛИВОЕ СТИРАНИЕ. Записка
					-- строкой выше намеренно НЕ перечислена, чтобы не затереться, — а связь с
					-- деталью затиралась даже тогда, когда кадр режется с видом detail и роль не
					-- меняется вовсе. Повторный разрез по тому же медиа уносил имя детали,
					-- которое человек вписал руками.
					detail_slot_id = IF(:is_detail, detail_slot_id, NULL),
					ordinal = VALUES(ordinal),
					set_by = VALUES(set_by), set_at = VALUES(set_at)`,
					map[string]any{
						"card": parent.TechCardId, "media": f.MediaId,
						"role": f.ViewKey, "ord": ord, "who": req.Actor,
						// Разрез не знает адреса на верстаке, поэтому «деталь» здесь значит только
						// «роль не изменилась» — то есть очищать нечего.
						"is_detail": f.ViewKey == entity.DesignViewDetail,
					}); err != nil {
