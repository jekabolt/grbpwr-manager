...
// видеть не должен. DefaultCompletionBudget заведена ровно ради этого случая; тот, кто переменную
// однажды поставит, обязан прийти сюда. Запас в полторы минуты — не защита от этого, а буфер на
// сборку промпта.
//
// ⚠ ЧТО ПРО ЭТУ ПЕРЕМЕННУЮ ИЗВЕСТНО, А ЧТО ТОЛЬКО ИЗМЕРЕНО РУКАМИ. Здесь стояло «ни один из двух
// spec'ов (бета, прод) её не ставит», и репозиторий этого утверждения НЕ ПОДДЕРЖИВАЕТ: в git лежит
// РОВНО ОДИН спек — .do/app.yaml, и он ПРОДОВЫЙ (domains: backend.grbpwr.com). Он действительно
// оставляет MODEL / BASE_URL / HTTP_TIMEOUT кодовым умолчаниям и говорит это вслух. Спек беты
// (.do/app-beta.yaml) в .gitignore — он живёт только на платформе DO, и из исходников его не
// проверить ничем. Живой спек беты прочитан на платформе руками 2026-09-04: из семейства
// OPENROUTER_* там заданы ТОЛЬКО API_KEY и MODEL_ANALYSIS, то есть HTTP_TIMEOUT не задан
// и сегодня лиза действительно длиннее бюджета вызова. Это СНИМОК, а не инвариант: он протухнет в
// ту минуту, когда переменную поставят на платформе, и никакой тест этого не увидит. Инвариант же
// держит не он, а вывод ниже — лиза считается той же функцией и из тех же потолков, что и провод.
//
// ⚠ И ВЫВОДИТСЯ ОНА ИЗ МАКСИМУМА ПО ВЕТКАМ, А НЕ ИЗ ПОТОЛКА ОДНОЙ. Здесь стояло
// entity.DesignConstructionMaxTokens — потолок СТРУКТУРНОЙ ветки, — а на провод кладётся
// entity.DesignDraftAnswerCeiling(ветка), потому что потолок это решение владельца ПРО ФОРМУ
// ОТВЕТА, и у прозы он однажды появится. Сегодня оба числа совпадают (у прозы потолка нет), и
// именно поэтому расхождение пришло бы МОЛЧА: в день, когда прозаический потолок встанет выше
// 8000, CompletionBudget обгонит лизу, один client_request_id заплатит дважды, а проба, спросившая
// константу одной ветки, останется зелёной. Цель коммита, который вывел лизу из бюджета, была
// названа дословно — «между ними нет ни одного второго числа»; максимум по таблице веток и есть
// эта цель на обеих осях.
var HandlerLease = HandlerLeaseFor(entity.DesignDraftAnswerCeilings()...)

// HandlerLeaseFor — лиза при ДАННОМ наборе потолков ответа: бюджет САМОГО ДОРОГОГО из них плюс
// запас. Ноль в списке («у ветки потолка нет») законен и означает базу бюджета.
//
...
// никакое сравнение значений их не различало. Здесь же максимум можно СПРОСИТЬ подставным
// списком: TestHandlerLeaseOutlivesTheLongestPaidCall поднимает каждую позицию по очереди и
// требует, чтобы лиза сдвинулась, — то есть краснеет ровно тогда, когда анкер снова садится на
// одну ветку.
func HandlerLeaseFor(answerCeilings ...int) time.Duration {
	longest := 0
	...
	return openrouter.DefaultCompletionBudget(longest) + designHandlerLeaseSlack
}
