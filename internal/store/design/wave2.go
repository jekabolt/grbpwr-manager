...
// ЛИЗА ХЕНДЛЕРА — СРОК, НА КОТОРЫЙ StartRun ОТДАЁТ СТРОКУ kind=draft_idea ТОМУ, КТО ЕЁ ОТКРЫЛ.
//
...
// и стала ЛОЖЬЮ НА 26.7 s в тот день, когда потолок ответа купил себе время. Оси, по которым эти
// два числа успели разойтись, ТРИ, и закрыты они по очереди:
//
//	ПОТОЛОК   — лиза считается той же CompletionBudget и тем же потолком, что и провод;
//	ВЕТКА     — не потолком СТРУКТУРНОЙ ветки, а МАКСИМУМОМ по entity.DesignDraftAnswerCeilings,
//	            иначе потолок, появившийся у прозы, обгонит лизу молча;
//	БАЗА      — и вот она была последней и самой тихой (см. ниже).
//
// ⚠ БАЗА: ЛИЗА БОЛЬШЕ НЕ СЧИТАЕТСЯ ЗДЕСЬ ВОВСЕ, И ЭТО ТРЕТЬЯ ОСЬ. Здесь стояло
// `openrouter.DefaultCompletionBudget(...)` — то есть КОДОВАЯ база в 60 s, — тогда как на проводе
// база берётся из cfg.HTTPTimeout (openrouter.New → budgetBase). Совпадали они ровно потому, что
// OPENROUTER_HTTP_TIMEOUT никто не задал, и этот замер стоял в комментарии как честная, но
// НЕСУЩАЯ подпорка. У соседнего клиента переменная УЖЕ задана (240 s), и на этом числе
// арифметика такова: вызов 240 + 8000/30 = 506.67 s против лизы 60 + 266.67 + 90 = 416.67 s — на
// 90-й секунде ВНУТРИ платного вызова строка свободна, повтор её перехватывает и платит второй
// раз. Ломается всё, начиная с базы в 150 s, и ни одна проба этого не видела: все они спрашивали
// ту же кодовую базу.
//
...
func (s *Store) StartRun(ctx context.Context, req entity.DesignRunStart) (*entity.DesignRunStarted, error) {
	...
	// ⚠ ЛИЗА У ТЕКСТОВОГО ПРОГОНА ОБЯЗАТЕЛЬНА, И ЕЁ ОТСУТСТВИЕ — ОТКАЗ, А НЕ УМОЛЧАНИЕ. Строку
	// этого рода исполняет ХЕНДЛЕР, и захват ей выдаёт только эта функция; сколько он продлится,
	// знает лишь тот, кто держит клиента поставщика (довод целиком — у HandlerLeaseFor). Подставив
	// здесь своё число, стор вернул бы ровно тот дефект, ради которого поле заведено: кодовую базу
	// в 60 s против настроенной на проводе — и повтор того же ключа платил бы модели второй раз.
	// Остальные рода лизу не носят: их строки забирает ClaimRuns своей.
	if req.Kind == entity.DesignRunKindDraftIdea && req.HandlerLease <= 0 {
		return nil, fmt.Errorf("%w: a %s run must carry a handler lease — without it the row is free "+
			"while the paid call is still running, and a repeat of the same client_request_id pays twice",
			entity.ErrDesignInvalidArgument, req.Kind)
	}
	...
	err := s.txFunc(ctx, func(ctx context.Context, rep dependency.Repository) error {
		...
		*out = entity.DesignRunStarted{}
		...
		if prior, ok, err := priorStart(ctx, db, req); err != nil {
			...
			resumed, run, err := resumeHandlerRun(ctx, db, prior, req.HandlerLease)
			if err != nil {
				return err
			}
		...
		if req.Kind == entity.DesignRunKindDraftIdea {
			...
			claimExpires = s.Now().UTC().Add(req.HandlerLease)
		}
