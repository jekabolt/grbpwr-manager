...
--   double_counted  — scalar AND an authored BOM row in the same section (this card's unit cost was
--                     silently double-counting; scalar is cleared, the BOM row wins);
...
-- double_counted: an AUTHORED row in the section (the sentinel name excludes rows a crashed earlier
-- run of THIS migration inserted, so a re-run cannot misclassify a half-migrated card).

INSERT IGNORE INTO tech_card_costing_migration_exception (tech_card_id, article, kind, amount, currency, approval_state)
SELECT tc.id, 'hardware', 'double_counted', c.hardware_cost, NULLIF(c.currency, ''), tc.approval_state
FROM tech_card tc JOIN tech_card_costing c ON c.tech_card_id = tc.id
WHERE COALESCE(c.hardware_cost, 0) <> 0 AND tc.approval_state = 'draft'
  AND EXISTS (SELECT 1 FROM tech_card_bom_item b WHERE b.tech_card_id = tc.id AND b.section = 'hardware'
...
FROM tech_card tc JOIN tech_card_costing c ON c.tech_card_id = tc.id
WHERE COALESCE(c.packaging_cost, 0) <> 0 AND tc.approval_state = 'draft'
  AND EXISTS (SELECT 1 FROM tech_card_bom_item b WHERE b.tech_card_id = tc.id AND b.section = 'packaging'
              AND b.name <> 'Packaging (migrated from costing)');

-- --- the migration itself: draft + colourways + NO row in the section -> synthetic BOM line ---
...
WHERE COALESCE(c.hardware_cost, 0) <> 0 AND tc.approval_state = 'draft'
  ...
  AND NOT EXISTS (SELECT 1 FROM tech_card_bom_item b WHERE b.tech_card_id = tc.id AND b.section = 'hardware');
...
WHERE COALESCE(c.packaging_cost, 0) <> 0 AND tc.approval_state = 'draft'
  AND EXISTS (SELECT 1 FROM product p WHERE p.style_id = tc.id)
  AND NOT EXISTS (SELECT 1 FROM tech_card_bom_item b WHERE b.tech_card_id = tc.id AND b.section = 'packaging');
...
WHERE b.name IN ('Hardware (migrated from costing)', 'Packaging (migrated from costing)')
  AND NOT EXISTS (SELECT 1 FROM tech_card_colorway_usage u
                  WHERE u.colorway_id = p.id AND u.bom_item_id = b.id);

-- clear the migrated scalars: draft cards whose section now carries a row (synthetic or authored —
-- the latter is the recorded double_counted population, where the BOM row wins)
