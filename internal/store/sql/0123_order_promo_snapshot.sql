...
--
-- Renumbered from the original 0121 (task 05): the master merge (auto-delivery) took 0121 for
-- shipment_carrier columns, so this is re-added as 0123. order/update.go writes these columns on
-- checkout and ~30 metrics queries read them, so the schema must carry them or both paths 1054.
--
