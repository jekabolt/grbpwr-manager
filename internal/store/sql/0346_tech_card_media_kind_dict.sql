...
--
-- ЗАМЕР НЕ СНЯТ. Размер `tech_card_media` (число строк и мегабайты) померить не удалось: кластер
-- пускает только по списку доверенных источников, IP машины разработки в него не входит, и
-- `mysql` отваливается с `ERROR 2003 … (60)` — это firewall, а не пароль. Добавление своего IP в
-- доверенные — изменение настройки безопасности, которое делается владельцем, а не миграцией.
-- Поэтому файл отделён ИМЕННО ДЛЯ ТОГО, чтобы его можно было вынуть из волны одним движением
-- (удалить файл; ни один другой файл волны его не читает и от него не зависит) и увезти в тихое
-- окно. Порог решения, когда замер появится: если копия таблицы не укладывается в МИНУТУ при
-- пятиминутном потолке всего прогона — файл едет отдельно.
--
