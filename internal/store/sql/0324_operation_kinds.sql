...
-- 10. СВИДЕТЕЛЬСТВО О МАСШТАБЕ. Ни один шаг файла не ПЕРЕПИСЫВАЕТ ЗНАЧЕНИЯ — ни одного UPDATE'а
--     в файле нет вовсе, ни одна старая строка после него не читается иначе. Но «не трогает
--     значения» это НЕ «не трогает строки», и назвать файл перестройкой одних метаданных было бы
--     неправдой. Замерено на одноразовом MySQL 8.0.46 (mysql:8.0, utf8mb3) тем же ALTER'ом, что
--     стоит в шаге 2:
--       * шаг 1 (MODIFY topstitch_mode VARCHAR(8) -> VARCHAR(16)) — INSTANT отвергнут, проходит
--         ALGORITHM=INPLACE: «0 rows affected», id tablespace'а не меняется. Вот ЭТО метаданные;
--       * шаг 2 (32 ADD COLUMN и 32 ADD CONSTRAINT ... CHECK ОДНИМ оператором) — MySQL отвергает
--         И INSTANT, И INPLACE со словами «Try ALGORITHM=COPY»: у ADD CONSTRAINT CHECK в 8.0
--         поддержан только COPY, а из спецификаций одного оператора движок берёт самую строгую.
--         Значит таблица КОПИРУЕТСЯ целиком: «Records: 400000», id tablespace'а меняется. Тот же
--         ADD COLUMN без CHECK'ов пошёл бы INSTANT — цену назначает именно CHECK;
--       * шаги 3..9 (DROP CHECK + ADD CONSTRAINT CHECK словарей) — по той же причине COPY.
--     Стоимость поэтому ЛИНЕЙНА по числу строк перестраиваемой таблицы. Тот же ALTER шага 2,
--     wall-clock целиком с клиентом: 0.11 с на пустой таблице, 0.97 с на 400 тыс. строк, 3.84 с
--     на 1.6 млн (~2.4 мкс/строку). tech_card_operation у нас на порядки меньше, поэтому и на
--     бете, и на проде это секунды, и пятиминутный дедлайн store.New держится с запасом.
--     ЧЕГО ЗДЕСЬ ЧИТАТЬ НЕЛЬЗЯ: «ADD COLUMN вместе с CHECK'ом дёшев». Он дёшев на НАШЕЙ таблице и
--     ровно поэтому. Тот, кто перенесёт приём на таблицу в десятки миллионов строк, получит полную
--     копию под блокировкой записи, а не перестройку метаданных; там CHECK'и добавляют отдельным
--     ALTER'ом после того, как ADD COLUMN прошёл INSTANT'ом.
