-- +migrate Up
-- Widen tech_card.aux_subtype with `garment_case` (кофр) — the carrier a garment travels/hangs in.
-- It is its own sub-type rather than a flavour of `dust_bag`: a кофр is cut, sewn and costed as its
-- own item (own BOM, pieces, operations), and the assembly bill has to be able to say which of the
-- two a style ships with.
--
-- Only the value-set CHECK moves. The purpose gate (chk_tech_card_aux_subtype_purpose, 0173) is
-- untouched, no column changes, no backfill: `garment_case` is a value operators pick going forward.
-- Existing rows classified as dust_bag are NOT re-read as кофр — the 0173 name heuristic mapped
-- "garment bag" to dust_bag, and silently re-labelling shipped classifications would rewrite what an
-- assembly bill already means. Re-file those by hand if any turn out to be кофр.
--
...
SET @has_stale := (SELECT COUNT(*) FROM information_schema.CHECK_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE() AND CONSTRAINT_NAME = 'chk_tech_card_aux_subtype'
      AND CHECK_CLAUSE NOT LIKE '%garment_case%');
...
SET @has_new := (SELECT COUNT(*) FROM information_schema.CHECK_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE() AND CONSTRAINT_NAME = 'chk_tech_card_aux_subtype'
      AND CHECK_CLAUSE LIKE '%garment_case%');

SET @sql := IF(@has_new = 0,
    'ALTER TABLE tech_card ADD CONSTRAINT chk_tech_card_aux_subtype CHECK (aux_subtype IS NULL OR aux_subtype REGEXP ''^(brand_label|care_label|size_label|hangtag|sticker|dust_bag|garment_case|box|insert|hanger|other)$'')',
    'SELECT 1');
PREPARE s FROM @sql;
EXECUTE s;
DEALLOCATE PREPARE s;
