...
-- 
-- ПУСТЫЕ СТРОКИ ЭТОЙ ШАПКИ — «-- » С ПРОБЕЛОМ, А НЕ ГОЛОЕ «--», И ЭТО НЕ МУСОР. sqlparse
-- пропускает строку как комментарий, только когда она начинается с «-- »; голое «--» попадает в
-- буфер оператора и живёт там до ближайшего «;». Пока за блоком комментариев идёт оператор, это
-- сходит с рук (и сходило по всем миграциям годами) — но ровно на этом 2026-08-16 не поднялось
-- приложение, когда из 0306 убрали последний оператор Up. Пробел стоит ноль и снимает вопрос.
-- 
