...
-- 8c. Колонки долой вместе с их CHECK-ом. Имя и номер уходят навсегда — переиспользовать их под
--     новым смыслом было бы тем самым молчаливым дрейфом, против которого написана шапка 0289.
SET @ddl := IF(@has_pressing = 1,
