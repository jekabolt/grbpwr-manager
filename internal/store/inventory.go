...
// Inventory returns an object implementing Inventory interface
func (ms *MYSQLStore) Inventory() dependency.Inventory {
...
func (is *inventoryStore) GetInventoryHealth(ctx context.Context, from, to time.Time, limit int) ([]entity.InventoryHealthRow, error) {
	...
	result, err := QueryListNamed[entity.InventoryHealthRow](ctx, is.DB(), query, map[string]any{
...
func (is *inventoryStore) GetSizeRunEfficiency(ctx context.Context, from, to time.Time, limit int) ([]entity.SizeRunEfficiencyRow, error) {
	...
	result, err := QueryListNamed[entity.SizeRunEfficiencyRow](ctx, is.DB(), query, map[string]any{
		"baseCurrency": baseCurrency,
		"from":         from,
		"to":           to,
		"limit":        limit,
