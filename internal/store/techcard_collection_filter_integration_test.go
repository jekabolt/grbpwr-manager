...
func TestListTechCardsCollectionFilter(t *testing.T) {
	...
	// ВЕДУЩИЙ ПРОБЕЛ: КАРТА ОБЯЗАНА НАХОДИТЬСЯ ПО СВОЕМУ ТОЧНОМУ ЗНАЧЕНИЮ.
	//
	// Пул значений фасета клиент собирает из тех же хранимых строк, что приезжают в строке листа,
	// поэтому в пуле окажется " SS25" дословно, и клик пошлёт его дословно. Обрежь сервер запрос —
	// и карта исчезла бы при фильтре по СВОЕМУ ЖЕ значению: на utf8mb3 сравнение PAD SPACE
	// добивает хвостовые пробелы, но ведущий не трогает никогда, так что " SS25" = "SS25" ложно.
	//
	// Фильтр обязан быть самосогласован с пулом: что отдали — тем и фильтруем.
	padded := " " + "ПРОБЕЛ-" + suffix
	cardD := insertCollectionCard(ctx, t, "cf-d-"+suffix, padded)
	got = byName(padded)
	require.Len(t, got, 1, "карта с ведущим пробелом обязана находиться по своему точному значению")
	require.Equal(t, cardD, got[0].Id)
	require.Equal(t, padded, got[0].Collection.String, "в пул фасета едет строка КАК ХРАНИТСЯ, вместе с пробелом")
}
