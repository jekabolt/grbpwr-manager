...
//
// ЖИВОЙ ПРОГОН МУТАЦИИ ПО САМОМУ ФАЙЛУ (2026-08-22), сверх постоянного теста выше — потому что
// постоянный тест ломает РАЗОБРАННУЮ структуру, а этот доказал, что красноту даёт и порча
// НАСТОЯЩЕГО sql-файла, вместе с разбором:
//
//	убраны две строки синонимов ('fold', 'сложить') и ('fold', 'складывание');
//	токен работы 'press_stretch' задвоен в 'press_flat'
//	→ TestOperationWorkCatalogSeed: «дубль токена работы: press_flat», «работа fold: нет ни
//	  одного кириллического синонима», плюс четыре «синоним … ссылается на неизвестную работу
//	  press_stretch»;
//	→ TestOperationWorkTokenVerbPairsAreFrozen: хеш сдвинулся;
//	→ TestOperationWorkCatalogGuardsAreNotFalseGreen: упал на своей же преамбуле «исходный сид
//	  обязан быть чистым».
