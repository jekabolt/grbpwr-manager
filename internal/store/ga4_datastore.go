...
// GA4Data returns an object implementing the GA4DataStore interface.
func (ms *MYSQLStore) GA4Data() dependency.GA4DataStore {
	return &ga4Store{MYSQLStore: ms}
}

// SaveGA4DailyMetrics saves or updates GA4 daily metrics.
func (s *ga4Store) SaveGA4DailyMetrics(ctx context.Context, metrics []ga4.DailyMetrics) error {
	if len(metrics) == 0 {
		return nil
	}
	...
	if err := BulkUpsert(ctx, s.DB(), "ga4_daily_metrics", cols, updCols, args); err != nil {
...
func (s *ga4Store) SaveGA4ProductPageMetrics(ctx context.Context, metrics []ga4.ProductPageMetrics) error {
	if len(metrics) == 0 {
		return nil
	}
	...
	if err := BulkUpsert(ctx, s.DB(), "ga4_product_page_metrics", cols, updCols, args); err != nil {
...
func (s *ga4Store) SaveGA4CountryMetrics(ctx context.Context, metrics []ga4.CountryMetrics) error {
	if len(metrics) == 0 {
		return nil
	}
	...
	if err := BulkUpsert(ctx, s.DB(), "ga4_country_metrics", cols, updCols, args); err != nil {
...
func (s *ga4Store) SaveGA4EcommerceMetrics(ctx context.Context, metrics []ga4.EcommerceMetrics) error {
	if len(metrics) == 0 {
		return nil
	}
	...
	if err := BulkUpsert(ctx, s.DB(), "ga4_ecommerce_metrics", cols, updCols, args); err != nil {
...
func (s *ga4Store) SaveGA4RevenueBySource(ctx context.Context, metrics []ga4.RevenueSourceMetrics) error {
	if len(metrics) == 0 {
		return nil
	}
	...
	if err := BulkUpsert(ctx, s.DB(), "ga4_revenue_by_source", cols, updCols, args); err != nil {
...
func (s *ga4Store) SaveGA4ProductConversion(ctx context.Context, metrics []ga4.ProductConversionMetrics) error {
	if len(metrics) == 0 {
		return nil
	}
	...
	if err := BulkUpsert(ctx, s.DB(), "ga4_product_conversion", cols, updCols, args); err != nil {
...
func (s *ga4Store) GetGA4DailyMetrics(ctx context.Context, from, to time.Time) ([]ga4.DailyMetrics, error) {
	query := `
		SELECT
...
func (s *ga4Store) GetGA4ProductPageMetrics(ctx context.Context, from, to time.Time, limit int) ([]entity.ProductViewMetric, error) {
	query := `
		SELECT
			p.id AS product_id,
...
func (s *ga4Store) GetGA4SessionsByCountry(ctx context.Context, from, to time.Time, limit int) ([]entity.GeographySessionMetric, error) {
	query := `
		SELECT
			country,
			SUM(sessions) AS total_sessions,
